...
def read_samples(read_samples_queue, source_audio_segments, target_audio_segments):
    while True:
        for batch_idx in range(len(source_audio_segments)):
            ...
            before = np.stack((librosa.amplitude_to_db(np.abs(before)) / 10, np.angle(before)), axis=0)
            after= np.stack((librosa.amplitude_to_db(np.abs(after)) / 10, np.angle(after)), axis=0)
