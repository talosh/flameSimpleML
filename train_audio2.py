...
try:
    checkpoint = torch.load('train_log_audio/model2_audio_training.pth')
...
def read_samples(read_samples_queue, source_audio_segments, target_audio_segments):
    while True:
        for batch_idx in range(len(source_audio_segments)):
            before = librosa.stft(source_audio_segments[batch_idx], n_fft=2047, hop_length=512, center=False)
            after = librosa.stft(target_audio_segments[batch_idx], n_fft=2047, hop_length=512, center=False)
            before = np.pad(before, ((0, 0), (0, 6)), mode='constant', constant_values=0)
            after = np.pad(after, ((0, 0), (0, 6)), mode='constant', constant_values=0)
            ...
            before_db = librosa.amplitude_to_db(np.abs(before)) / 10
            after_db = librosa.amplitude_to_db(np.abs(after)) / 10

            before = np.stack((before_db, np.angle(before)), axis=0)
            after= np.stack((after_db, np.angle(after)), axis=0)
...
while epoch < num_epochs + 1:
    ...
    for batch_idx in range(len(source_audio_segments)):
        ...
        if step % 99 == 1:
            ...
            sample_before_mag = librosa.db_to_amplitude( sample_before[0] * 10 )
            ...
            # sample_before = sample_before[0] + 1j * sample_before[1]
            output_audio_before = librosa.istft(sample_before, n_fft=2047, hop_length=512, center=False)
            sf.write('test_audio2/01_before.wav', output_audio_before, sr)

            sample_after = after[0].clone().cpu().detach().numpy()[:, :-6]
            sample_after_mag = librosa.db_to_amplitude( sample_after[0] * 10 )
            ...
            # sample_after = sample_after[0] + 1j * sample_after[1]
            output_audio_after = librosa.istft(sample_after, n_fft=2047, hop_length=512, center=False)
            sf.write('test_audio2/02_after.wav', output_audio_after, sr)

            sample_current = output[0].clone().cpu().detach().numpy()[:, :-6]
            sample_current_mag = librosa.db_to_amplitude( sample_current[0] * 10 )
            ...
            # sample_current = sample_current[0] + 1j * sample_current[1]
            output_audio_current = librosa.istft(sample_current, n_fft=2047, hop_length=512, center=False)
            sf.write('test_audio2/03_output.wav', output_audio_current, sr)
        ...
        print (f'\rEpoch [{epoch + 1} - {days:02}d {hours:02}:{minutes:02}], Time:{data_time_str} + {train_time_str}, Batch [{batch_idx + 1} / {len(source_audio_segments)}], Lr: {optimizer.param_groups[0]["lr"]:.4e}, Loss L1: {loss_l1_str}', end='')
        step = step + 1
    ...
    }, f'train_log_audio/model2_audio_training.pth')
