...
def read_samples(read_samples_queue, source_audio_segments, target_audio_segments):
    while True:
        for batch_idx in range(len(source_audio_segments)):
            ...
            before_db = librosa.amplitude_to_db(np.abs(before)) / 9
            after_db = librosa.amplitude_to_db(np.abs(after)) / 9
...
while epoch < num_epochs + 1:
    ...
    for batch_idx in range(len(source_audio_segments)):
        ...
        if step % 99 == 1:
            ...
            sample_before_mag = librosa.db_to_amplitude( sample_before[0] * 9 )
            ...
            sample_after_mag = librosa.db_to_amplitude( sample_after[0] * 9 )
            ...
            sample_current_mag = librosa.db_to_amplitude( sample_current[0] * 9 )
